...
public class StartGameFace extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		totalScore.setText("总得分：0");
		totalScore.setTextColor(Color.BLACK);
		RelativeLayout.LayoutParams totalScoreParams = new RelativeLayout.LayoutParams(
				50, 20); // 设置按钮的宽度和高度
		// i表示的是第i行 k表示的是第k列
		totalScoreParams.topMargin = btnHeight * (numHeiNum + calHeiNum + splitHeight * 2) + btnHeight + 10; // 纵坐标定位
		totalScoreParams.leftMargin = 0; // 横坐标定位
		relativeLayout.addView(totalScore, totalScoreParams);
		
		TextView totalCnt = new TextView(this);
		totalCnt.setText("总成功次数：0");
		RelativeLayout.LayoutParams totalCntParams = new RelativeLayout.LayoutParams(
				50, 20); // 设置按钮的宽度和高度
		// i表示的是第i行 k表示的是第k列
		totalCntParams.topMargin = btnHeight * (numHeiNum + calHeiNum + splitHeight * 2) + btnHeight + 10; // 纵坐标定位
		totalCntParams.leftMargin = 50 + 10; // 横坐标定位
			...
			if (leftBtn == 0) {
				...
				if (!"=".equals(expressBtnArr[widNum - 2].getText())) {
					...
					try {
						...
						// 计算的结果不相等
						if (Integer.parseInt(expressBtnArr[widNum - 1].getText().toString()) != Integer.parseInt(result.toString())) {
